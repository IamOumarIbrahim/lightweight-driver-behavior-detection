...
Run `export_rgb_predictions.bat` once to sanitize the six frozen local RGB YOLO
result files into public, path-safe prediction envelopes. Run `analyze_rgb.bat`
to reproduce the structural annotation audit, macro, per-class, per-subject
operating metrics, and paired seed differences without training or inference.
