...
class DFineAdapter(DetectorAdapter):
    ...
    def load(self) -> DFineAdapter:
        checkout = THIRD_PARTY_ROOT / "D-FINE"
        if not checkout.is_dir():
            raise ProtocolError(
                "Pinned D-FINE checkout is missing; run scripts/setup/02_setup_backends.bat"
            )
        ...
        if (mismatched or missing) and not self.allow_pretrained_head_mismatch:
            raise ProtocolError(
                f"D-FINE checkpoint is not a strict {self.class_count}-class checkpoint: "
                f"{len(mismatched)} mismatched, {len(missing)} missing"
            )
        self.model.load_state_dict(
            matched, strict=not self.allow_pretrained_head_mismatch
        )
    ...
    def normalize(self, raw_outputs: Any, image_ids: list[int]) -> list[dict[str, Any]]:
        ...
        for image_id, image_labels, image_boxes, image_scores in zip(
            ...
            for label, box, score in zip(
                image_labels.cpu(), image_boxes.cpu(), image_scores.cpu()
            ):
