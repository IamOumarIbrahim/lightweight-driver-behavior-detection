...
\begin{itemize}
    ...
    \item a checksum-backed reproduction workflow plus a clearly separated, one-seed, currently five-system NIR training-negative exposure study with deterministic 1-FPS midpoint sampling.
\end{itemize}
...
Ten protected NIR passes are complete. Table~\ref{tab:nirresults} reports both exposures for YOLO11n, YOLO26n, D-FINE-N, SSDLite-MobileNetV3-Large, and RT-DETRv2-S, while Fig.~\ref{fig:nir} plots the same checksum-backed results. YOLOX-Nano, YOLOv10n, and YOLOv8n remain pending and are not inferred.

Increasing negative exposure from 1:2 to 1:6 changes $\mathrm{mAP}_{50:95}$ by $-5.55$, $+5.52$, $-0.08$, $+2.32$, and $+0.37$ percentage points for YOLO11n, YOLO26n, D-FINE-N, SSDLite-MobileNetV3-Large, and RT-DETRv2-S, respectively. Micro-$F_1$ improves only for SSDLite-MobileNetV3-Large and decreases for the other four systems. The direction is therefore model-dependent rather than a uniform benefit of 1:6 exposure. RT-DETRv2-S has the highest strict-IoU AP and micro-$F_1$ at both ratios in the completed subset, with 24.66--25.61-ms tensor-to-final p50 latency.
...
\caption{Ten NIR protected-test results at seed 13 and deterministic 1-FPS midpoint sampling. Grouped columns compare each completed model at 1:2 and 1:6 training exposures; the dark-grey rule separates the ratio sections, and each thick red segment marks the five-model mean for its ratio.}
\label{fig:nir}
\end{figure}

\section{Discussion}
\subsection{Accuracy--Efficiency Trade-offs}
No detector is universally best: YOLO11n leads coarse-IoU mAP and throughput, YOLO26n leads strict-IoU mAP and compactness, and D-FINE-N leads micro- and macro-$F_1$ but runs slowest in RGB. In the completed NIR subset, RT-DETRv2-S leads strict-IoU AP and micro-$F_1$ at both exposures, while SSDLite-MobileNetV3-Large is faster but has lower AP and more false detections. Native training and postprocessing make these shipped-system comparisons, not isolations of architecture.
